...
test('combination-level instructions keep conflict, repair and decision paths distinct', () => {
  ...
  assert.equal(focus(gentleDirect, '你别一上来就不高兴，我又不是故意的。', triggerTurn)?.secondary?.label, '表达直接');
  assert.match(instructions(gentleDirect, '你别一上来就不高兴，我又不是故意的。', triggerTurn), /“不是故意”与实际影响的区别/);
  ...
  assert.match(instructions(quickClose, '确实是我没提前说，害你等了这么久，怪我。'), /略带保留的接受结束追责/);
  assert.match(instructions(gentleDirect, '确实是我没提前说，害你等了这么久，怪我。'), /不主动提出吃饭、拥抱或后续安排/);
  assert.match(instructions(playfulClose, '确实是我没提前说，害你等了这么久，怪我。'), /直接接受认错并结束冲突/);
...
test('server envelope compiles abstract phase and combination behavior without fixed dialogue', () => {
  ...
  assert.deepEqual(envelope.personality, { primary: '情绪退得快', secondary: '表达直接' });
  assert.match(envelope.reply_shape, /直接接受认错并结束冲突/);
  ...
  assert.match(decisionEnvelope.reply_shape, /低风险共同梗、生活小细节或轻微夸张/);
...
test('resolved boundary lets fast recovery own a later affection turn and marks reopening', () => {
  ...
  assert.equal(slowRecovery?.primary.label, '表达直接');
  assert.equal(slowRecovery?.secondary, null);
  assert.match(personalityTurnFocusEnvelope(slowRecovery!).reply_shape, /不附加条件、不引用旧冲突/);
