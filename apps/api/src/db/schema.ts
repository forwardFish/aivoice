...
export const voiceProfiles = pgTable('voice_profiles', {
  ...
  voiceAddress: text('voice_address').notNull().default(''),
  userAddress: text('user_address').notNull().default(''),
  ageYears: integer('age_years'),
  gender: text('gender'),
  userAgeYears: integer('user_age_years'),
  ...
  check('voice_profiles_voice_address_length', sql`char_length(${table.voiceAddress}) <= 10`),
  ...
  check('voice_profiles_user_age_years_valid', sql`${table.userAgeYears} IS NULL OR (${table.userAgeYears} >= 0 AND ${table.userAgeYears} <= 120)`),
