...
test('P0 screens keep their primary actions and navigation visible on compact viewports', () => {
  ...
  assert.match(selectVideoView, /<app-button[^>]*label="下一步"/)
  ...
  assert.match(clipView, /<bottom-action-bar[^>]*custom-class="clip-footer"/)
  assert.match(clipView, /label="\{\{saving \? '正在保存片段…' : '下一步'\}\}"/)
  assert.match(bottomBarStyle, /env\(safe-area-inset-bottom\)/)
  ...
  assert.match(purchaseView, /<bottom-action-bar[^>]*custom-class="purchase-footer"/)
...
test('P1 screens use simplified hierarchy while retaining real business actions', () => {
  ...
  assert.match(profileView, /<bottom-action-bar[^>]*custom-class="profile-footer"/)
  assert.match(profileView, /bindaction="submit"/)
