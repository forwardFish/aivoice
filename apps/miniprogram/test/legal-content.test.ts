...
test('legal docs are product-specific and routed from app.json', () => {
  ...
  assert.match(legalMarkup, /open-type="feedback"/)
...
test('legal documents cover all account entry types', () => {
  ...
  assert.ok(ai.sections.some(section => section.heading.includes('合规')))
  for (const doc of [help, service, feedback, dataPrivacy, rules, privacy, terms, ai]) {
    assert.equal(doc.version, 'v1.0')
    assert.doesNotMatch(JSON.stringify(doc), /v0\.9-draft|开发版|本地验证版|产品化草案|公开上线前/)
  }
  assert.equal(feedback.feedbackAction, true)
  ...
  assert.match(privacyText, /厦门市海沧区熵之跳动五金店/)
  assert.match(privacyText, /闽ICP备2026017884号-2X/)
