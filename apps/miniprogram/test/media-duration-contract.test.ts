...
test('creation flow uses one 8-60 second video and an 8-20 second cloning reference', () => {
  ...
  assert.match(selectClipWxml, /已自动选择清晰片段/)
  assert.match(selectClipWxml, /系统已锁定质量最高的连续片段/)
  assert.match(selectClipWxml, /wx:if="\{\{!autoClipSelected\}\}" class="advanced-toggle"/)
  assert.match(selectClipTs, /Number\(session\.clipStartMs \|\| 0\) \/ 1000/)
  assert.match(selectClipTs, /Number\(session\.durationMs\) \/ 1000/)
  assert.match(selectClipTs, /if \(this\.data\.autoClipSelected\) return/)
