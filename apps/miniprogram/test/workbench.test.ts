...
test('pending chat keeps the user message out of the assistant bubble', () => {
  ...
  assert.match(source, /chatText: '',\s*chatCount: 0,\s*chatInputFocused: false,\s*chatKeyboardHeight: 0,\s*chatComposerStyle: '',\s*bottomAnchorId: submittedBottomAnchorId,/)
...
test('chat mode keeps nav and top chrome outside the scrolling message list while exact mode keeps its own scroll container', () => {
  ...
  assert.match(markup, /<view wx:if="\{\{mode === 'chat'\}\}" class="chat-panel">[\s\S]*<scroll-view[\s\S]*class="messages-scroll \{\{chatViewportReady \? 'is-ready' : ''\}\}"/)
  assert.match(markup, /class="messages-scroll \{\{chatViewportReady \? 'is-ready' : ''\}\}"[\s\S]*bounces="\{\{false\}\}"[\s\S]*enhanced="\{\{true\}\}"/)
  assert.doesNotMatch(markup, /<scroll-view[\s\S]*class="messages-scroll \{\{chatViewportReady \? 'is-ready' : ''\}\}"[\s\S]*class="segment-control"/)
