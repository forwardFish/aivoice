...
test('all clip range update paths keep waveform selection and markers synchronized', async () => {
  ...
  instance.applyMarkerPosition('start', 25, { left: 0, width: 100 })
  assert.equal(instance.data.startSec, 10)
  assert.equal(instance.data.endSec, 25)
  instance.applyMarkerPosition('end', 100, { left: 0, width: 100 })
  assert.equal(instance.data.endSec, 30)
  assert.equal(instance.data.endSec - instance.data.startSec, 20)
  instance.updateRange(5, 25, 40)
...
test('WXML binds live percentages and quota dialog avoids immediate-arrival promise', () => {
  ...
  assert.match(view, /data-edge="start"[^>]+catchtouchmove="onMarkerDragMove"/)
  assert.match(view, /data-edge="end"[^>]+catchtouchmove="onMarkerDragMove"/)
  assert.match(style, /\.marker[^}]+pointer-events:\s*auto/)
