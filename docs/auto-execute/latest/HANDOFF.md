...
- API tests: 27 PASS, 2 environment-gated skips.
...
- Worker tests: 22 PASS, 2 environment-gated skips.
...
3. Virtual-payment callback signature, AES GET/POST and sandbox order creation pass. Android sandbox payment confirmation remains manual.
4. iOS has no virtual-payment sandbox. Android sandbox must pass first, then iOS must use a real CNY 9.90 order.
5. Preserved old CloudBase, Tencent Cloud and Bailian keys remain pending revocation after final confirmation.

## Next action

Complete Android sandbox payment on the generated preview, verify the 50-point grant and callback idempotency, then revoke preserved old credentials and publish the production virtual good.
