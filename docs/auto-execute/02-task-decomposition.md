...
| PC-RISK-001 | Control | Codex serial owner | Freeze `67f92ac`, create backup and risk-first branch, define no-ICP gates | Git + acceptance docs | none | branch refs, diff, typecheck | PASS |
| PC-RISK-002 | Gateway/Auth | Codex serial owner | Deploy NestJS as a CloudBase HTTP Function and invoke through `wx.cloud.callHTTPFunction` with injected OpenID | API entry, auth, mini-program transport | PC-RISK-001 | local contract tests + live invocation | IN_PROGRESS |
| PC-RISK-003 | Media | Codex serial owner | Upload authorized video with native cloud storage and play generated/preview audio through cloud file download without a custom domain | runtime storage adapter, Worker, mini-program audio | PC-RISK-002 | <=100MB upload, metadata, playback, delete evidence | PENDING |
| PC-RISK-004 | Jobs | Codex serial owner | Prove immediate per-job invocation plus timer recovery works with no resident Run dispatcher | API job dispatch + Worker timer | PC-RISK-002 | direct invoke, retry, dedupe and stalled recovery tests | PENDING |
| PC-RISK-005 | Payment | Codex serial owner | Use CloudBase HTTP Function/Integration callback domain for JSAPI order and apply `+50` through the existing atomic RPC exactly once | payment function/config/RPC | PC-RISK-002 | crypto tests + live small payment + replay | PENDING |
| PC-RISK-006 | Gate | Codex serial owner | Run contract, integration, secret and report checks; block the remaining migration unless all four risk gates pass | full risk-first diff | PC-RISK-002..005 | scoped risk final report | PENDING |
