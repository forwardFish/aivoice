...
> 账户采用统一积分余额，所有声音共享。新注册账号仅赠送一次 5 积分；每次成功生成扣 1 积分，失败不扣；¥9.9 购买 50 积分，可重复购买，用完再买且不自动续费。注册送分、单次消耗、商品价格、商品积分数和有效期均由后端配置，客户端不得自行计算或记账。积分流水须区分 `REGISTER_GRANT`、`PURCHASE_GRANT`、`GENERATION_CONSUME`、`REFUND`、`MANUAL_ADJUSTMENT`，并预留 `INVITE_GRANT`；本版本不实现邀请功能。本文后续出现的“免费 1 次”“¥9.9/10 次”“按声音免费/付费分账”等旧描述全部废止，由本段统一替代。
...
> 产品形态：微信小程序  
> 运营主体：个体工商户  
> MVP 售价：¥9.9 / 10 次成功的 AI 语音输出  
> 免费体验：每个微信账号最多赠送 1 次自定义 AI 语音生成  
> 文档目标：以最小开发范围验证用户在完成固定试听和 1 次个性化语音体验后，是否愿意支付 ¥9.9 继续使用熟悉的私有 AI 声音。
...
固定试听用于验证声音效果；免费自定义生成用于验证用户输入自己的内容后，产品是否真正有价值。
...
优先使用免费体验额度
→ 免费额度为 0 后使用付费额度
→ 两者都为 0 时返回 QUOTA_EXHAUSTED
```

系统增加额度流水，记录赠送、购买、成功消耗、退款和人工调整，避免重复支付回调、并发生成和售后补偿造成账目不一致。
